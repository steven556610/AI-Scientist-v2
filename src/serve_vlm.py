#!/usr/bin/env python
"""vLLM-backed OpenAI-compatible server for Qwen2.5-VL-32B-Instruct.
...
streaming, multi-image content parts, `tools`/`tool_choice` -- is provided by
vLLM's own entrypoint, so this file is only argument translation plus an exec.

What that buys over the old server:
  * continuous batching, so the `_GEN_LOCK` that serialised every request is
    gone. Concurrent tree-search workers no longer queue behind each other.
  * paged KV cache preallocated at startup. The old server sized nothing up
    ...
    --gpu-memory-utilization and a request that does not fit is rejected, not
    fatal.
  * `tools`/`tool_choice` are parsed instead of silently dropped by a Pydantic
    model that had no field for them.
...
usage:
    ...
    SERVE_DRY_RUN=1 python src/serve_vlm.py --port 8000
...
Environment (same contract as the transformers version, plus three new ones):
    ...
    SERVE_MAX_MEM         per-GPU cap, e.g. "72GiB". Translated to a
                          --gpu-memory-utilization fraction against the card's
                          total. Required in practice on a SHARED card: vLLM
                          otherwise claims 90% of the whole GPU at startup and
                          takes memory the experiment workers need.
    SERVE_MAX_MODEL_LEN   context window, prompt + generation. Default 32768.
    ...
    SERVE_MAX_PIXELS      cap on image resolution fed to the vision tower, via
                          --mm-processor-kwargs. A full-size plot becomes
                          thousands of visual tokens; this is the usual cause
                          of a VLM OOM at request time. Default 1280*28*28.
    SERVE_MAX_IMAGES      images accepted per prompt. vLLM defaults to 1, which
                          would reject the multi-plot feedback requests this
                          pipeline sends. Default 8.
...
Weights are ~64 GB in bf16, so SERVE_MAX_MEM must leave room for the KV cache
on top of that.
"""
...
# (backend_openai.py:240), so it asks for "qwen2.5-vl-32b". vLLM validates the
# requested name against what it serves and 404s on a mismatch -- unlike the
# old server, which ignored req.model entirely. Every alias in use must be
# listed or the pipeline breaks at the first VLM call.
SERVED_NAMES = [
    "qwen2.5-vl-32b",
...
def parse_size_to_bytes(text: str) -> int:
    """Parse "72GiB" / "70GB" / "70000MiB" / "12345678" into bytes."""
    text = text.strip()
    units = {
...
def gpu_memory_utilization() -> "float | None":
    ...
    card is shared with a 88 GB server plus the experiment workers -- so the
    translation has to happen somewhere. Here.
    """
...
def limit_mm_flag(n_images: int) -> "list[str]":
    """--limit-mm-per-prompt, whose syntax changed across vLLM releases.

    vLLM <0.9 wants repeated `image=N`; 0.9+ wants a JSON object. Detected
    rather than guessed, because getting it wrong is a startup crash and the
    installed version is not pinned by this repo.
    """
    try:
        import vllm  # noqa: F401  (version probe only)
        major, minor = (int(p) for p in vllm.__version__.split(".")[:2])
        json_form = (major, minor) >= (0, 9)
    except Exception:
        json_form = True
    if json_form:
        return ["--limit-mm-per-prompt", f'{{"image": {n_images}}}']
    return ["--limit-mm-per-prompt", f"image={n_images}"]


def build_argv(host: str, port: int, passthrough: "list[str]") -> "list[str]":
    n_gpu = visible_gpu_count()
    max_pixels = int(os.environ.get("SERVE_MAX_PIXELS", DEFAULT_MAX_PIXELS))
    max_images = int(os.environ.get("SERVE_MAX_IMAGES", DEFAULT_MAX_IMAGES))
    max_model_len = int(os.environ.get(
        "SERVE_MAX_MODEL_LEN",
        max(DEFAULT_MAX_MODEL_LEN,
            int(os.environ.get("SERVE_MAX_NEW_TOKENS", 0)) + 4096),
    ))
    ...
    argv = [
        ...
        "--trust-remote-code",
        # Qwen2.5-VL's processor takes the pixel budget directly; this is the
        # vLLM equivalent of AutoProcessor(max_pixels=...) in the old server.
        "--mm-processor-kwargs", f'{{"max_pixels": {max_pixels}}}',
    ]
    argv += limit_mm_flag(max_images)
    if n_gpu > 1:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Serve Qwen2.5-VL-32B through vLLM's OpenAI API.")
